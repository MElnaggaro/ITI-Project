...
def test_classify_request_intents():
    ...
    assert classify_request("SELECT * FROM orders", database_connection_ids=[uuid4()]) == "database"
    ...
    assert classify_request("Summarize company report PDF", knowledge_base_ids=[uuid4()]) == "document"
