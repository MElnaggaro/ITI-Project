...
- [ ] Create the migration in foreign-key-safe order: extensions; tenants; users; roles; user_roles; database_connections; database_schemas; database_tables; database_columns; table_permissions; column_permissions; knowledge_bases; files; document_chunks; conversations; messages; query_executions; message_citations; audit_logs.
- [ ] Define the downgrade/reversal order child-first and review it as a development/schema test operation only; do not treat a destructive downgrade on a populated production platform as a business-data management mechanism.
- [ ] Map every listed table in SQLAlchemy 2 without replacing database-enforced primary keys, unique constraints, check constraints, nullable columns, server defaults, or ON DELETE behavior with weaker application-only validation.
- [ ] Use server-side UUID and timestamp defaults exactly where Section 7 supplies them; do not add an undocumented updated_at trigger as if it were in the PDF. If automatic update maintenance is later added, record it as a compatible extension and test it.
- [ ] Map JSONB defaults as database defaults, preserving '{}'::jsonb versus '[]'::jsonb exactly; avoid mutable process-level default objects that differ from the database contract.
- [ ] Ensure nullable fields remain nullable where the reference DDL omits NOT NULL, including database_connections.schema_sync_status, database_tables.schema_id, database_columns.is_nullable, document_chunks.embedding, and audit_logs.tenant_id/user_id.
- [ ] Retain only the two explicitly declared non-unique indexes, idx_users_tenant_id and idx_database_connections_tenant, in the base schema; any additional performance index must be a separately reviewed extension with migration and test.
- [ ] Place connection credentials only in database_connections.encrypted_password or encrypted_connection_string after Phase 05 encryption; never add plaintext secret columns, sample business rows, or raw connection diagnostics to the schema.
- [ ] Treat database_columns.sample_values as metadata-only: Phase 06 must apply an approved small cap, sensitive-value masking, tenant approval/policy, retention, and audit controls before storage.
- [ ] Treat query_executions.result_preview as absent by default and, when explicitly enabled, as redacted, bounded, retention-controlled audit metadata rather than durable customer business data.
- [ ] Keep source database keys/relationship metadata in database_schemas, database_tables, and database_columns; do not copy full source tables, records, or unbounded query results into application PostgreSQL.
- [ ] Validate the selected embedding provider produces exactly 1024 dimensions before document_chunks.embedding or Qdrant points are written; fail processing safely on mismatch and do not permit divergent dual-index state.
- [ ] Add repository/query conventions in later phases that always scope tenant-owned tables by trusted tenant_id, including tables whose PK is addressed directly; a foreign key alone is not authorization.
- [ ] Record every non-Section 7 table/field/index/trigger introduced later as an extension with owner phase, migration revision, purpose, privacy impact, and rollback/retention decision.
...
- [ ] Add a clean-database Alembic upgrade test that creates uuid-ossp and vector and reaches the exact latest schema revision.
- [ ] Add a schema-introspection contract test for all 18 tables: column names, PostgreSQL types, nullable flags, server defaults, primary keys, unique constraints, check constraints, foreign keys, and every ON DELETE action.
- [ ] Assert the exact explicit indexes idx_users_tenant_id on users(tenant_id) and idx_database_connections_tenant on database_connections(tenant_id), and assert no required index was replaced by a non-equivalent definition.
- [ ] Assert tenants.code, uq_users_tenant_email, uq_roles_tenant_name, uq_database_connection_name, uq_database_schema, uq_database_table, uq_database_column, uq_column_permission, uq_knowledge_base_name, and uq_document_chunk all enforce the stated unique behavior.
- [ ] Assert user_roles has its exact composite primary key and table_permissions rejects both-null and both-populated role_id/user_id subjects.
- [ ] Add cascade/set-null lifecycle tests for every Section 7 relationship, including self-referential messages.parent_message_id and nullable audit_logs tenant/user references.
- [ ] Add server-default tests for UUID generation, JSONB object/array defaults, booleans, statuses, timestamps, and VECTOR(1024) type dimension.
- [ ] Add tenant-isolation repository tests for every tenant-owned table and direct-ID lookup tests for database_connections, files, conversations, messages, citations, query executions, and audits.
- [ ] Add source-data-residency tests proving schema cache samples and result previews are bounded/masked/disabled by default and that bulk source rows cannot be persisted or vectorized.
- [ ] Add Qdrant/pgvector mirror tests for idempotent create, reprocess, delete, 1024-dimension validation, and failed-sync reconciliation behavior.
- [ ] Add migration upgrade/downgrade tests on disposable development databases and an extension-compatibility test that prevents silent destructive schema drift.

## Definition of Done

- [ ] An Alembic migration and SQLAlchemy 2 metadata define all 18 Section 7 core tables plus uuid-ossp and vector without omissions or weakened semantics.
- [ ] The base schema preserves every listed field, data type, nullability rule, default, key, FK ON DELETE action, check/unique constraint, and the two explicit indexes.
- [ ] document_chunks.embedding remains VECTOR(1024), Qdrant is documented as the canonical retrieval index, and synchronization/mismatch behavior has a Phase 13 owner.
- [ ] The schema contains no plaintext source-credential field, copied customer business table, unbounded business-row cache, or undocumented extension.
- [ ] Schema-contract, migration, relationship, tenant-isolation, data-residency, and vector-mirror tests are specified and assigned to the automated suite.
