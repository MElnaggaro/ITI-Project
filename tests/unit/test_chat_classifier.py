...
def test_classify_request_intents():
    ...
    # 1. Database intent by keywords or connection_ids
    assert classify_request("SELECT * FROM orders") == "database"
    assert classify_request("Show total sales", connection_ids=[uuid4()]) == "database"
    ...
    # 3. Hybrid intent
    assert classify_request("Compare sales database with PDF report", connection_ids=[uuid4()], knowledge_base_ids=[uuid4()]) == "hybrid"
